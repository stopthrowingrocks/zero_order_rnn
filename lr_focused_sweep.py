#!/usr/bin/env python3
"""
Focused geometric sweep of learning rate between 0.0373 and 0.2683
to map out the convergence region in detail with 16 values.
"""
import os
import time
import numpy as np
...
def main():
    ...
    # Fixed SPSA hyperparameters (using baseline)
    epsilon = 0.1
    num_perturbations = 8
    batch_size = 16

    # Focused learning rate sweep: 16 values between 0.0373 and 0.2683
    lr_min = 0.0373
    lr_max = 0.2683
    lr_values = np.geomspace(lr_min, lr_max, 16)

    output_dir = 'hpp-opt'
    os.makedirs(output_dir, exist_ok=True)

    print("="*80)
    print("FOCUSED LEARNING RATE SWEEP (16 values)")
    print("="*80)
    ...
    print(f"Fixed: epsilon={epsilon}, perturbations={num_perturbations}, batch={batch_size}")
    ...
    for i, lr in enumerate(lr_values):
        print(f"[{i+1}/16] Testing LR={lr:.4f}...", end=" ", flush=True)
    ...
    fig.suptitle(f'Focused Learning Rate Sweep (16 values)\nTask: Reverse (seq={min_seq_length}-{max_seq_length}, vocab={vocab_size})\n'
                 f'Fixed: epsilon={epsilon}, pert={num_perturbations}, batch={batch_size}', fontsize=12)
    ...
    # Plot 3: Steps to convergence
    ax = axes[1, 0]
    if converged_lrs:
        converged_steps = [r['steps'] for r in results if r['converged']]
        ax.scatter(converged_lrs, converged_steps, c='green', s=100, alpha=0.6)
        ax.plot(converged_lrs, converged_steps, 'g--', alpha=0.3)
    ax.set_xscale('log')
    ax.set_xlabel('Learning Rate', fontsize=10)
    ax.set_ylabel('Steps to Convergence', fontsize=10)
    ax.set_title('Steps to Convergence (Converged Runs Only)', fontsize=11)
    ax.grid(True, alpha=0.3)

    # Plot 4: Final loss (all runs)
    ax = axes[1, 1]
    ax.scatter(lrs, final_losses, c=colors, s=100, alpha=0.6)
    ax.axhline(y=convergence_loss, color='blue', linestyle='--', linewidth=2, label=f'Convergence threshold ({convergence_loss})')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlabel('Learning Rate', fontsize=10)
    ...
    plt.tight_layout()
    plot_filename = f'{output_dir}/focused_lr_sweep_16values_vocab{vocab_size}_seq{min_seq_length}-{max_seq_length}.png'
    ...
    print(f"Converged: {num_converged}/16 ({100*num_converged/16:.1f}%)")
    ...
    if num_converged > 0:
        ...
        print(f"  Best steps: {min(converged_steps)} at LR={converged_lrs[converged_steps.index(min(converged_steps))]:.4f}")

    print("="*80)

    # Send Telegram notification
    if TELEGRAM_AVAILABLE:
        message = f"""**Focused Learning Rate Sweep Complete!**

Task: Reverse (seq={min_seq_length}-{max_seq_length}, vocab={vocab_size})

**Results:**
- Tested 16 LR values from {lr_min:.4f} to {lr_max:.4f}
- Converged: {num_converged}/16 ({100*num_converged/16:.1f}%)

Fixed: epsilon={epsilon}, pert={num_perturbations}, batch={batch_size}"""
